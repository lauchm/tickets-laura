...
<style>
  ...
  .rec-card{{background:#fff;border:1.5px solid #D8DCE6;border-radius:8px;padding:0.85rem 1rem;margin-bottom:8px;display:flex;align-items:center;gap:10px;}}
  .rec-card:hover{{border-color:{NAVY};}}
  .rec-name{{font-size:14px;font-weight:600;color:#1A202C;flex:1;}}
  .rec-count{{font-size:12px;color:#8892A4;}}
  .rec-last{{font-size:11px;color:#8892A4;}}
  .hist-row{{background:#F7F8FA;border-radius:6px;padding:5px 10px;margin-bottom:4px;font-size:12px;color:#4A5568;display:flex;gap:8px;align-items:center;}}
...
def get_sheet_tickets():
    ...
    try:
        ...
        ws = sh.add_worksheet("Tickets", rows=2000, cols=8)
...
def get_sheet_recurrentes():
    ...
    try:
        ...
        ws = sh.add_worksheet("Recurrentes", rows=500, cols=3)
        ws.append_row(["ID","Nombre","Área"])
    return ws
...
def get_sheet_historial():
    ...
    try:
        ...
        ws = sh.add_worksheet("Historial", rows=5000, cols=4)
...
def load_historial():
    ...
    data = ws.get_all_records()
    if not data:
        return pd.DataFrame(columns=["ID","Fecha","Tarea","Área"])
    return pd.DataFrame(data)
...
<div class="cohimar-header">
  ...
  <div class="hdr-lines"><div class="l-red"></div><div class="l-wh"></div><div class="l-nv"></div></div>
</div>
""", unsafe_allow_html=True)
...
# ── PESTAÑAS ──────────────────────────────────────────────────────────────────
tab1, tab2 = st.tabs(["🗂️ Tickets", "🔁 Tareas recurrentes"])
...
with tab1:
    ...
    if st.session_state.editing_id is not None:
        ...
        if not row.empty:
            row = row.iloc[0]
            st.markdown("---")
            st.markdown(f"### ✏️ Editando: {row['Título']}")
            with st.form("editar_ticket"):
                ...
                e_notas = st.text_area("📝 Notas adicionales / documentación posterior",
                    value=row.get("Notas",""), height=100,
                    placeholder="Resultados finales, acuerdos, seguimiento posterior...")
    ...
    if not df.empty:
        ...
        with fc1: f_area   = st.multiselect("Filtrar por área", AREAS)
        with fc2: f_estado = st.multiselect("Filtrar por estado", ESTADOS)
        with fc3: f_buscar = st.text_input("🔍 Buscar")
        df_fil = df.copy()
        ...
        with col:
            ...
            if tickets_col.empty:
                ...
                for _, row in tickets_col.iterrows():
                    ...
                    btns = st.columns(4)
                    with btns[0]:
                        if st.button("✏️", key=f"edit_{tid}", help="Editar"):
                            st.session_state.editing_id = tid; st.rerun()
    ...
    if not df.empty:
        ...
        ec1, ec2 = st.columns([3,1])
        with ec1:
            st.caption(f"📋 {n_total} tickets · {n_done} resueltos")
        with ec2:
...
with tab2:
    ...
    st.markdown("### 🔁 Tareas recurrentes")
    st.caption("Define aquí tus tareas del día a día. Cada vez que la hagas, pulsa '✓ Hecha hoy' y queda registrada con la fecha de hoy automáticamente.")

    # Añadir nueva recurrente
    with st.expander("➕  Nueva tarea recurrente", expanded=False):
        with st.form("nueva_rec", clear_on_submit=True):
            rc1, rc2 = st.columns([3,1])
            with rc1:
                r_nombre = st.text_input("Nombre de la tarea *", placeholder="Ej: Revisión copias de seguridad")
            with rc2:
                r_area = st.selectbox("Área *", AREAS)
            r_sub = st.form_submit_button("✓ Crear tarea recurrente", use_container_width=True)
            if r_sub:
                if not r_nombre.strip():
                    st.error("Escribe un nombre.")
                else:
                    add_recurrente(r_nombre.strip(), r_area)
    ...
    if df_rec.empty:
        st.info("Aún no tienes tareas recurrentes. Añade la primera arriba.")
    else:
        for _, rec in df_rec.iterrows():
            rid   = rec["ID"]
            nombre = rec["Nombre"]
            area   = rec["Área"]
            color  = AREA_COLORS.get(area,"#4A5568")

            # Calcular veces hechas y última vez
            if not df_hist.empty:
                hist_tarea = df_hist[df_hist["Tarea"]==nombre]
                veces = len(hist_tarea)
                ultima = hist_tarea["Fecha"].max() if veces > 0 else None
            else:
                veces = 0
                ultima = None
            ...
            # Ya hecha hoy?
            hecha_hoy = False
            if not df_hist.empty and veces > 0:
                hecha_hoy = str(date.today()) in df_hist[df_hist["Tarea"]==nombre]["Fecha"].values

            c1, c2, c3, c4 = st.columns([3, 1.2, 1, 0.5])
            with c1:
                st.markdown(f"""
                <div style="display:flex;align-items:center;gap:8px;padding:6px 0;">
                  <span class="area-badge" style="background:{color};padding:2px 8px;border-radius:20px;font-size:11px;font-weight:700;color:#fff;">{area}</span>
                  <span style="font-size:14px;font-weight:600;color:#1A202C;">{nombre}</span>
                </div>""", unsafe_allow_html=True)
            with c2:
                st.markdown(f'<div style="font-size:12px;color:#8892A4;padding-top:10px;">✓ {veces} veces · última: {ultima or "—"}</div>', unsafe_allow_html=True)
            with c3:
                if hecha_hoy:
                    st.success("✓ Hecha hoy")
                else:
                    if st.button("✓ Hecha hoy", key=f"hecha_{rid}"):
                        registrar_hecha(nombre, area)
                        st.success(f"✅ '{nombre}' registrada.")
                        st.rerun()
            with c4:
                if st.button("🗑", key=f"delrec_{rid}"):
                    delete_recurrente(rid)
                    st.rerun()

        # Historial
        st.markdown("---")
        st.markdown("### 📅 Historial de tareas realizadas")

        if df_hist.empty:
            st.info("Aún no hay registros en el historial.")
        else:
            # Filtro por tarea
            tareas_unicas = ["Todas"] + sorted(df_hist["Tarea"].unique().tolist())
            f_tarea = st.selectbox("Filtrar por tarea", tareas_unicas)
            df_hist_fil = df_hist if f_tarea=="Todas" else df_hist[df_hist["Tarea"]==f_tarea]
            df_hist_fil = df_hist_fil.sort_values("Fecha", ascending=False)

            # Resumen
            rc1, rc2 = st.columns([2,1])
            with rc1:
                st.caption(f"📋 {len(df_hist_fil)} registros")
            with rc2:
                csv_h = df_hist_fil.to_csv(index=False).encode("utf-8")
                st.download_button("↓ Exportar historial", data=csv_h,
                                   file_name="historial_recurrentes_2026.csv",
                                   mime="text/csv", use_container_width=True)

            for _, h in df_hist_fil.head(50).iterrows():
                color_h = AREA_COLORS.get(h["Área"],"#4A5568")
                st.markdown(f"""
                <div class="hist-row">
                  <span style="font-size:11px;color:#8892A4;min-width:80px;">{h["Fecha"]}</span>
                  <span class="area-badge" style="background:{color_h};padding:1px 7px;border-radius:20px;font-size:10px;font-weight:700;color:#fff;">{h["Área"]}</span>
                  <span style="font-size:13px;color:#1A202C;">{h["Tarea"]}</span>
                </div>""", unsafe_allow_html=True)

            if len(df_hist_fil) > 50:
                st.caption(f"Mostrando los últimos 50 de {len(df_hist_fil)} registros. Exporta el CSV para verlos todos.")
